...
A buddy allocator is implemented for contiguous physical memory allocation.

- Contiguous multi-frame allocation
- Power-of-two block sizes
- Block splitting
- Buddy block coalescing
- `buddytest`

#### Slab Allocator

A slab allocator is implemented for small kernel allocations.
...
Files use eight direct data-block pointers first. Once those blocks are full,
the inode allocates a single 4 KiB indirect block containing 32-bit data-block
addresses. This provides an addressable maximum of 1024 indirect entries
(4 MiB of data in the addressing scheme), subject to the physical 1 MiB RAM
disk capacity.

#### Hierarchical directories

Directories contain `.` and `..` entries and can be nested. Path resolution
supports absolute and relative paths, including `.` and `..`.
...
| `write <file> <text>` | Create/truncate and write text |
| `rm <path>` | Remove a file or empty directory |
| `mkdir <path>` | Create a directory |
| `cd <path>` | Change current directory |
| `pwd` | Print current directory |
| `fstest` | Automated five-file create/write/read/delete test |

#### Write-ahead journaling

`ramdisk.c` implements a small write-ahead journal using the final two RAM-disk
blocks. Before a filesystem block is changed, its before-image is recorded in
the journal. The transaction is then committed only after the target block has
been written. On initialization, an uncommitted transaction is rolled back.
This gives the RAM filesystem a crash-recovery mechanism without `malloc` or
libc.

#### VFS abstraction

`fs.h` defines `file_ops_t`, a filesystem operation vtable containing open,
read, write, close, unlink, mkdir, chdir, list and pwd operations. `vfs_root()`
exposes the active RAM filesystem through this interface so a different
filesystem implementation can be plugged in later.
...
For the required five-file deliverable, run:

```text
fstest
```

The command creates, writes, reads, and deletes five files and prints `ls`
after every create, write, read, and delete operation.

### Stage 4 extension test
...
Tag the completed implementation as:

```bash
git add .
git commit -m "Implement Stage 4 RAM disk filesystem and extensions"
git tag v0.5-stage4
