...
uint32_t scheduler_fork(uint32_t *context_esp)
{
    ...
    child->pid = 0;
    /* Allocate a fresh stack slot without going through process_create(). */
    child->stack_base = PROCESS_STACK_POOL + child_index * STACK_SIZE;
