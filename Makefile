...
KERNEL_C_SRCS := kernel/kernel.c kernel/vga.c kernel/keyboard.c kernel/process.c kernel/scheduler.c kernel/thread.c kernel/mutex.c kernel/semaphore.c kernel/rwlock.c kernel/deadlock.c kernel/stage2_demo.c kernel/pmm.c kernel/buddy.c kernel/slab.c kernel/ramdisk.c kernel/fs.c
...
.PHONY: all clean run debug gdb grub grub-run info
all: $(OS_IMAGE)
	@echo ""
...
debug: $(KERNEL_ELF)
	...
	qemu-system-i386 -drive format=raw,file=$(OS_IMAGE) -m 32M -s -S
